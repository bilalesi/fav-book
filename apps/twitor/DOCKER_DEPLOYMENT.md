...
2. **Twitor Service**: Twitter bookmark crawler
   ...
   - Port: 8001
   - Depends on: postgres, valkey, server, restate
...
- `RESTATE_INGRESS_URL`: Restate ingress URL (e.g., `http://restate:8080`)
- `RESTATE_ADMIN_URL`: Restate admin URL (e.g., `http://restate:9070`)
...
The dev environment includes Twitor, Valkey, PostgreSQL, and Restate in Docker:
...
4. **Restate** - Must be healthy before Twitor starts
...
- `bookmarks-prod-restate-data`: Restate workflow data

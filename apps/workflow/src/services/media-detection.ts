...
import type { IMediaDetectionResult } from "../types";

export interface IMediaDetectionInput {
	url: string;
	bookmarkId: string;
	workflowId: string;
	userId: string;
...
export const media_detection_service = restate.service({
	name: "media_detection_service",
	handlers: {
		detect: async (
			ctx: restate.Context,
			input: IMediaDetectionInput,
		): Promise<IMediaDetectionResult> => {

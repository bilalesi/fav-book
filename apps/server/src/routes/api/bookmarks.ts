import prisma, { type Prisma, ProcessingStatus } from "@favy/db";
import {
  DictPlatform,
  DictProcessingStatus,
  type IBookmarkEnrichmentInput,
  type TPlatform,
} from "@favy/shared";
import type { Prisma as PrismaType } from "@prisma/client";
import { z } from "zod";
import { protected_middleware } from "@/middleware/protected";
import { extract_metadata } from "@/tools/metadata-extractor";
import { validate_normalize_url } from "@/tools/url-normalizer";
import {
  has_enrichment_enabled,
  is_media_download_enabled,
} from "@/tools/utils";
import { createWorkflowClient } from "@/tools/workflow-factory";
...
async function spawn_enrichment_workflow(
  bookmarkId: string,
  userId: string,
  platform: TPlatform,
  url: string,
  ...
  try {
    ...
    const payload: IBookmarkEnrichmentInput = {
      bookmarkId,
      userId,
      platform,
      url,
    ...
    const handle = await workflowClient.invoke_bookmark_enrichment(payload);
...
export const bookmarks_router = {
  create: protected_middleware
    ...
    .handler(async ({ input, context }) => {
      ...
      const workflowId = await spawn_enrichment_workflow(
        bookmark.id,
        userId,
        input.platform,
        input.postUrl,
  ...
  list: protected_middleware
    ...
    .handler(async ({ input, context }) => {
      ...
      if (filters.excludeCategoryIds && filters.excludeCategoryIds.length > 0) {
        where.categories = {
          none: { categoryId: { in: filters.excludeCategoryIds } },
        };
      }
  ...
  bulkImport: protected_middleware
    .input(
  ...
  createUrlBookmark: protected_middleware
    .input(
      z.object({
        url: z.url(),
    ...
    .handler(async ({ input, context }) => {
      ...
      try {
        normalizedUrl = validate_normalize_url(input.url);
      } catch (error) {
        const errorMessage =
      ...
      if (!title || !description) {
        try {
          const metadata = await extract_metadata(normalizedUrl);
  ...
  checkUrlBookmarked: protected_middleware
    .input(
      z.object({
  ...
  bulkDelete: protected_middleware
    .input(
      z.object({
        bookmarkIds: z
          .array(z.string())
  ...
  bulkAddToCollections: protected_middleware
    .input(
      z.object({
        bookmarkIds: z
          .array(z.string())
  ...
  getEnrichmentStatus: protected_middleware
    .input(z.object({ id: z.string() }))
    .handler(async ({ input, context }) => {
      const userId = context.session.user.id;
  ...
  retryEnrichment: protected_middleware
    .input(z.object({ id: z.string() }))
    .handler(async ({ input, context }) => {
      const userId = context.session.user.id;
  ...
  batchImportUrls: protected_middleware
    .input(
      z.object({
        urls: z
    ...
    .handler(async ({ input, context }) => {
      ...
      for (const url of urls) {
        try {
          const normalized = validate_normalize_url(url);
      ...
      for (const { original, normalized } of validatedUrls) {
        try {
          ...
          try {
            const metadata = await extract_metadata(normalized);

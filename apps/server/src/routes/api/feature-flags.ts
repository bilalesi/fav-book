 ...
 */
export const feature_flags_router = {
	/**
	 * Get current feature flags
	 * Available to all authenticated users
	 */
	retrieve_flags: protected_middleware.handler(() => {
	 ...
	 */
	update_flags: protected_middleware
		.input(
			z.object({
		...
		.handler(async ({ context, input }) => {
			...
			const validationErrors = validateFeatureFlags(
				input as Partial<IFeatureFlags>,
			);
			...
			// Update the flags
			try {
				updateFeatureFlags(input as Partial<IFeatureFlags>);
	 ...
	 * Available to all authenticated users
	 */
	retrieve_flag: protected_middleware
		.input(
			z.object({
				name: z.enum([
	 ...
	 */
	is_admin: protected_middleware.handler(({ context }) => {
		const userEmail = context.session.user.email;
		return {

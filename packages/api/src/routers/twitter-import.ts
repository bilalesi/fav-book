...
const crawlStopResponseSchema = z.object({
  status: z.string(),
 ...
 */
async function proxyToTwitor<T>(
  endpoint: string,
  options: RequestInit = {}
): Promise<T> {
...
export const twitterImportRouter = {
  ...
  startCrawl: protectedProcedure
    ...
    .handler(async ({ input, context }) => {
      ...
      try {
        const response = await proxyToTwitor<
          z.infer<typeof crawlStartResponseSchema>
        >("/api/crawl/start", {
          method: "POST",
   ...
   */
  stopCrawl: protectedProcedure
    .input(z.object({ sessionId: z.string().min(1, "Session ID is required") }))
    .handler(async ({ input }) => {
      try {
        const response = await proxyToTwitor<
          z.infer<typeof crawlStopResponseSchema>
